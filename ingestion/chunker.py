...
class SemanticChunker:
    ...
    def _detect_is_heading(self, line: str, next_lines: List[str]) -> bool:
        """
        Detect if a line is a section heading.

        A heading is a short line (under 60 chars) that is followed within
        the next few lines by a longer paragraph line (over 60 chars).
        List items, numbers, and empty lines are excluded.
        """
        stripped = line.strip()
        if not stripped or len(stripped) >= 60:
            return False
        # Exclude list items and numbered items
        if stripped[0] in ('•', '-', '*') or stripped[0].isdigit():
            return False
        # Must be followed by at least one longer paragraph line nearby
        for next_line in next_lines[:4]:
            if next_line.strip() and len(next_line.strip()) > 60:
                return True
        return False

    def _section_aware_split(self, content: str) -> List[tuple]:
        """
        Split content into sections at detected headings.

        Returns a list of (section_heading, section_content) tuples.
        If no headings are detected, returns a single tuple with empty heading.
        """
        lines = content.split('\n')
        sections = []
        current_heading = ""
        ...
        for i, line in enumerate(lines):
            if self._detect_is_heading(line, lines[i + 1:]):
                if current_lines:
    ...
    async def chunk_document(
        ...
        """
        Chunk a document into semantically coherent pieces with section awareness.

        Splits content at detected section headings first, then applies semantic
        or simple chunking within each section. Adds section_heading and page_number
        (if present in metadata) to every chunk's metadata.

        Args:
            content: Document content (ideally a single page for page-number accuracy)
            title: Document title
            source: Document source
            metadata: Additional metadata (should include page_number for PDFs)
        ...
        for section_heading, section_content in sections:
            ...
            section_base_metadata = {
                **base_metadata,
                "section_heading": section_heading,
            }

            # Choose chunking strategy for this section
            if self.config.use_semantic_splitting and len(section_content) > self.config.chunk_size:
                try:
                    text_chunks = await self._semantic_chunk(section_content)
                except Exception as e:
                    logger.warning(f"Semantic chunking failed for section '{section_heading}', falling back: {e}")
                    text_chunks = self._simple_split(section_content)
            else:
                text_chunks = self._simple_split(section_content)
            ...
            section_chunk_objects = self._create_chunk_objects(
                text_chunks,
                section_content,
                section_base_metadata
            )

            # Re-index globally across all sections
            for chunk in section_chunk_objects:
                chunk.index = global_index
                global_index += 1

            all_chunks.extend(section_chunk_objects)

        # Update total_chunks count in all chunk metadata
        for chunk in all_chunks:
            chunk.metadata["total_chunks"] = len(all_chunks)

        return [
            c for c in all_chunks
            if len(c.content.strip()) >= self.config.min_chunk_size
        ]
