...
DROP INDEX IF EXISTS idx_page_embeddings_embedding;
DROP INDEX IF EXISTS idx_page_embeddings_document_id;
DROP INDEX IF EXISTS idx_page_embeddings_doc_page;
...
-- -------------------------------------------------------------------------
-- Page-level semantic index
-- Each row stores the full text and embedding for one page of a PDF.
-- document_id is nullable so pages can be stored before or without the
-- corresponding document record (e.g. from the standalone build script).
-- -------------------------------------------------------------------------
...
CREATE INDEX idx_page_embeddings_embedding ON page_embeddings
    USING ivfflat (embedding vector_cosine_ops) WITH (lists = 1);
CREATE INDEX idx_page_embeddings_document_id ON page_embeddings (document_id);
CREATE UNIQUE INDEX idx_page_embeddings_doc_page
