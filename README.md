...
Knowledge graph ingestion via Graphiti is LLM-call heavy and **scales with content density** — it is not a fixed number. Tracing `graphiti_core` 0.12.4's single-episode `add_episode` path, each chunk makes **3 fixed LLM calls** (entity extraction → node resolution → edge extraction) **plus 1 per extracted entity** (summary/attribute generation) **plus ~1 per extracted edge** (edge resolution, when candidate edges exist). That is a floor of ~4 for a sparse chunk and typically **~8–15** for a content-rich one. (Defaults matter: `MAX_REFLEXION_ITERATIONS=0` disables the extra reflexion passes, and `update_communities=False` skips community-update calls.) Across 552 chunks that is on the order of several thousand sequential LLM calls. At Groq's free-tier rate limits, this runs for many hours.
